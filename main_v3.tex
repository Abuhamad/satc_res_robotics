...
\;-\;\lambda_c\,\bigl\|\Phi(\bm{x}+\bm{\delta}_x,\bm{t}')-\Phi(\bm{x},\bm{t})\bigr\|_{2}
...
Here $\Phi$ is the model-specific coupling mechanism named above, namely FiLM in RT-1, the projection MLP in OpenVLA, and cross-attention in Octo, and $\lambda_c$ weights the coupling term so the visual and textual perturbations align at grounding rather than act independently.
...
For same-backbone pairs, the shared tokenizer transfers token-level edits directly. For closed-backbone victims with a substituted public VLM, tokenizers diverge and token-level edits restricts $\bm{t}'$ to paraphrase and suffix edits, whose effect is carried by semantics rather than token identity. We test this restriction empirically rather than assume it: the evaluation stratifies textual transfer success by surrogate-victim backbone mismatch and compares token-level edits against paraphrase and suffix edits under the same budget $\epsilon_t$, so the tokenizer-mismatch penalty is measured directly on the transferability matrix.
In the joint variant, $\mathcal{M}(\bm{x},\bm{t})=(\bm{x}+\bm{\delta}_x^{*},\bm{t}')$ solves \autoref{eq:abbp-obj-joint} on the surrogate with the surrogate coupling $\Phi'$, since the FiLM, projection-MLP, and cross-attention mechanisms named in \autoref{sec:t1_2} are all public components, and the alternating optimization and the ensemble of $K$ surrogates apply unchanged.
The budget split $(\epsilon_x,\epsilon_t)$ then becomes a transfer variable: we will report per-modality transfer success, \ie the fraction of episodes reaching $D_{\mathcal{M}}(H)\ge\rho$ under text-only, vision-only, and joint perturbations, to identify which channel carries transfer across surrogate-victim mismatch.
Both variants inherit the scope restriction above and apply only to language-conditioned models.
...
\BfPara{Proposed Attack} Alongside the language-based attacks we investigate (\eg SHIELD, \autoref{fig:lang_black_box}), we propose TTDA, a a query-based black-box attack against chunking policies, \ie policies predicting a sequence of $c$ actions per inference (\eg Octo, ACT). 
...
\BfPara{Related Work} Several existing defenses (\autoref{tab:defense}) were designed for VLMs. Recent work has begun to 
% JailGuard~\cite{zhang2023mutation} and ECSO~\cite{gou2024eyes} 
...
\BfPara{Proposed Defense 1} We propose TACD, an action-space anomaly detector operating on predicted actions rather than inputs. Because TACD inspects the action output directly, it complements input-side defenses and remains effective when the compromised modality is unknown. TACD operates at two levels. At the per-step level, it flags actions outside the feasible set $\mathcal{A}(\bm{\theta})$ of \autoref{eq:abbp-shift}, \ie actions violating joint limits, the reachable workspace, or the per-step velocity bound. At the sequence level, a multi-task trajectory-forecasting model conditioned on a task embedding, trained jointly on the benign demonstrations of all tasks, forecasts $\hat{\bm{a}}_{k:k+c}$ from the action history $\bm{a}_{k-c:k}$, where the horizon equals the policy chunk length $c$ of \autoref{sec:t2_2}, and raises an alarm when
\vspace{-1ex}\begin{equation}
  \small
s_k=\max_{k'\in[k,k+c)} \bigl\|W\bigl(\bm{a}_{k'}-\hat{\bm{a}}_{k'}\bigr)\bigr\|_{2},
\qquad
\mathrm{flag}_k=\mathbb{1}\bigl[\,\bm{a}_k\notin\mathcal{A}(\bm{\theta}_k)\ \lor\ s_k>\xi_t\,\bigr],
\label{eq:tacd-rule}
\vspace{-1.5ex}\end{equation}
...
\label{eq:agat-obj}
\vspace{-1.5ex}\end{equation}
where $\mathcal{D}_{\mathrm{ABBP}}$ is the perturbation distribution induced by \autoref{eq:abbp-obj} subject to the kinematic feasible set $\mathcal{A}(\bm{\theta})$ of \autoref{eq:abbp-shift}, mixing on-manifold bin-adjacent shifts ($\sigma{=}1$) with occasional larger boundary crossings ($\sigma{>}1$) so the model learns to resist various threat models. The same objective admits textual and joint perturbations drawn from \autoref{eq:abbp-obj-text} and \autoref{eq:abbp-obj-joint}, and we include them in the mixture, since textual augmentation requires no image rendering. AGAT and the attacks of \autoref{sec:t1} thus share one saddle point: the attack maximizes the margin of \autoref{eq:abbp-obj} while AGAT minimizes the task loss under the same perturbations. AGAT applies LoRA rank-16 adaptation to the action head and the final transformer blocks ($\sim$25M parameters, $\bm{\omega}_{\mathrm{LoRA}}$) rather than full-model tuning, keeping the defense compute-feasible and efficient.
...
\vspace{-1ex}\begin{equation}
  \small
\mathcal{V}(e)=\mathbb{1}\Bigl[\,\exists k:\ \bm{a}_k\notin\mathcal{A}(\bm{\theta}_k)\ \lor\ \bigl\|W\bm{e}_k\bigr\|_{2}>\rho_{\mathrm{safe}}\,\Bigr],
\label{eq:safety-violation}
\vspace{-1.5ex}\end{equation}
where $\mathcal{A}(\bm{\theta}_k)$ is the kinematic feasible set of \autoref{eq:abbp-shift}, $\bm{e}_k$ the weighted pose deviation of \autoref{eq:abbp-gate}, and $\rho_{\mathrm{safe}}$ a per-task safety margin calibrated from gripper clearance and object size, as for $\rho$ in \autoref{sec:t1}. Safety assessment is then the estimation of $\Pr[\mathcal{V}]$ per condition over the evaluation episodes, with conditions spanning the attack families of \autoref{sec:t1} and \autoref{sec:t2}, the defenses of \autoref{sec:t3_1}, and benign shifts in lighting, clutter, and instruction phrasing, so the assessment covers both adversarial and non-adversarial failure modes. Because $\mathcal{V}$ subsumes the per-step detector of \autoref{eq:tacd-rule}, the same protocol scores the defenses and certifies the detector, and the resulting violation rates form the safety benchmark of this task.
...
\cib{1} \emph{Benign safety:} $\hat p_q\le p_{0}$ with the upper confidence bound below $p_{0}$ in every benign cell, for a per-task rate $p_{0}$ fixed before experiments, \eg $p_{0}{=}0.01$ on the benchmark tasks.
\cib{2} \emph{Defense efficacy:} for each attack $\mathcal{M}$, the defended cells satisfy $\hat p_q^{\,\mathrm{def}}\le \gamma_{s}\,\hat p_q^{\,\mathrm{undef}}$ with $\gamma_{s}{=}0.5$, paralleling the cost criterion $\mathcal{C}_{\mathcal{M}}$ of \autoref{eq:defense-metrics} in terms of violations rather than attack budget, and the two criteria are reported together.
\cib{3} \emph{Detector consistency:} the TACD rule of \autoref{eq:tacd-rule} attains $\mathrm{TPR}\ge0.9$ against $\mathcal{V}$-positive episodes at the calibrated $1\%$ false-positive rate, which tests whether the detector of \autoref{sec:t3_1} certifies the violation event of this task.
