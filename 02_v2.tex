...
Let $\mathcal{F}$ emit, for each of the $d$ action dimensions, a categorical distribution over $B{=}256$ bins with logits $\bm{z}_j(\bm{x},\bm{t})\in\mathbb{R}^{B}$. The decoded bin and the decoded action are:
\begin{equation}
  \small
...
\emph{Stage 1 (target selection).} Given the current configuration $\bm{\theta}$, ABBP selects a coordinated bin-shift vector $\bm{s}\in\mathbb{Z}^{d}$ maximizing end-effector displacement under a stealth budget and a kinematic-feasibility constraint,
\begin{equation}
\bm{s}^{\star}=\argmax_{\substack{\bm{s}\in\mathbb{Z}^{d},\; s_j=0\;\forall j\notin\mathcal{J}}}
...
\label{eq:abbp-shift}
\end{equation}
subject to $\hat{\bm{a}}+\bm{s}\odot\bm{\Delta}\in\mathcal{A}(\bm{\theta})$, where $\mathcal{A}(\bm{\theta})$ contains the actions $\bm{a}$ whose executed configuration $\Psi(\bm{\theta},\bm{a})$ after one control period $\tau$ respects the joint limits $\Theta$ and the reachable workspace $\mathcal{W}$, and which satisfy the per-step velocity bound $\|\bm{a}-\hat{\bm{a}}\|_{\infty}\le\dot a_{\max}\tau$. The objective is convex in $\bm{s}$, so the $\ell_1$ budget $\beta$ alone would concentrate the whole shift in the single dimension of largest physical gain. The per-dimension cap $\sigma<\beta$ forces the shift across several degrees of freedom, and the bin bounds keep $b_j+s_j^{\star}$ decodable without post-hoc clipping.
\emph{Stage 2 (perturbation synthesis).} With $b_j^{\star}=b_j+s_j^{\star}$, ABBP solves a margin objective over the visual input,
\begin{equation}
\min_{\bm{\delta}}\;\sum_{j\in\mathcal{J}} w_j
\Bigl[\max_{c\neq b_j^{\star}} z_{j,c}(\bm{x}+\bm{\delta},\bm{t})-z_{j,b_j^{\star}}(\bm{x}+\bm{\delta},\bm{t})+\kappa\Bigr]_{+}
\quad\text{s.t.}\quad
\|\bm{\delta}\|_{\infty}\le\epsilon,\;\;\bm{x}+\bm{\delta}\in[0,1]^{n},
\label{eq:abbp-obj}
\end{equation}
with $[\cdot]_{+}=\max(\cdot,0)$, confidence margin $\kappa$, box constraint in raw pixel space before encoder normalization, and weights $w_j=\|(WG(\bm{\theta}))_{:,j}\|_{2}\Delta_j$ so the optimizer allocates perturbation budget to the dimensions with the largest physical effect per bin. \autoref{eq:abbp-obj} treats the $d$ logit vectors as conditionally independent, which holds for parallel readout heads ({\eg} RT-1, Octo). For autoregressive action decoders ({\eg} RT-2, OpenVLA), the action tokens form a sequence and $\bm{z}_j$ depends on the preceding tokens, so we evaluate each margin under teacher forcing on the target prefix $b^{\star}_{<j}$. We solve \autoref{eq:abbp-obj} by projected gradient descent with step $\alpha$ over $T$ iterations. Setting $w_j{=}1$ and $\sigma{=}1$ while retaining $\mathcal{A}(\bm{\theta})$ yields a discretization-agnostic variant, isolating the contribution of the physical-effect weighting in an ablation.

\emph{Comparison protocol.} PGD, FGSM, and C\&W serve as baselines, applied per step under the same budget. A one-bin action shift and a language-token substitution are not comparable units, so we compare attacks at matched physical consequence rather than at matched $\epsilon$. A single bin crossing displaces the end effector by a fraction of a millimetre, which no task-level metric resolves, so we measure consequence as the cumulative deviation of a closed-loop rollout rather than a one-step displacement. From a common initial state and instruction, we roll out the policy for $H$ steps twice, once on clean observations and once with the attack re-optimized at every step, and compare the two end-effector poses $(\bm{p}_k,R_k)$ and $(\bm{p}'_k,R'_k)$ obtained by forward kinematics,
\begin{equation}
