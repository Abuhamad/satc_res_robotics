...
\BfPara{Adoption and Constraints}
At inference time, each model must run within the robot's control-loop budget of 3--10~Hz. Small models such as ACT, Octo, and RT-1 can meet this rate locally on the robot, so we study them primarily under the white-box threat model with access to encoder parameters. Larger models such as OpenVLA and RT-2 (7B+ parameters) are hosted on a cloud service and queried over the network, which limits an adversary to inputs and outputs. We therefore treat these cloud-hosted models under the black-box threat model, matching how a real attacker would reach them. Our defenses respect the same budget: the trajectory-level detector TACD must add inference overhead small enough to keep the pipeline within the 3--10~Hz rate, or it is not deployable on the robot.
...
\BfPara{Relationship to Concurrent Work}
