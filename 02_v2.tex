...
\begin{wrapfigure}{r}{0.4\textwidth}{\vspace{-2ex}}
  \centering
...
\BfPara{Challenges in Attacking and Exploring Threat Space in Robotic Models} 
...
If Octo is used for robots/datasets that do not provide text instructions, the model processes available inputs and mask unavailable ones $\mathcal{F}(\{\bm{x}\}, \oslash,\{\bm{s}\})=\{\bm{a}_{out}\}$.
...
For example, $\mathcal{M}(\bm{x},\bm{t}) \xrightarrow{{}_{\textbf{U}}} \bm{a}_{out}'$ such that $\bm{a}_{out}' \neq \bm{a}_{out}$ is an untargeted evasion attack that alters the input pair $(\bm{x},\bm{t})$ to cause the model $\mathcal{F}$ to produce an incorrect output $\bm{a}_{out}'$.
Similarly, $\mathcal{M}(\bm{t}) \xrightarrow{{}_{\textbf{T}}} \bm{a}_{jailbreak}$ is a targeted jailbreak attack that alters the input instructions $(\bm{t})$ to cause the model $\mathcal{F}$ to produce a specific (undesired) output $\bm{a}_{jailbreak}$.
...
Most evasion attacks against VLM \cite{schlarmann2023adversarial, bailey2023image,gao2024adversarial,wang2024stop,tan2024wolf} typically use gradient-based approaches, \eg PGD and C\&W, to generate/optimize perturbation for adversarial samples.
Recent work has begun to move beyond static, single-frame formulations toward trajectory-level redirection and architecture-specific attacks designed to flow-matching robot policies, which we will build on and compare against~\cite{puthumanaillam2026trajectory,tae2026drift}.
...
Let the action vector $\mathcal{C}\subseteq\{1,\dots,d\}$ carry $d$ dimensions of continuous values (discrete dimensions are excluded)
 that are uniformly quantized into $B{=}256$ bins. For $j\in\mathcal{C}$, the model $\mathcal{F}$ outputs logits $\bm{z}_j(\bm{x},\bm{t})\in\mathbb{R}^{B}$, and the decoded bin and the decoded action are:
\begin{equation}
  \small
b_j=\argmax_{c\in\{0,\dots,B-1\}} z_{j,c}(\bm{x},\bm{t}),
\qquad
\hat a_j=l_j+\bigl(b_j+\tfrac{1}{2}\bigr)\Delta_j,
\qquad
\Delta_j=\frac{u_j-l_j}{B},
\label{eq:abbp-quant}
\vspace{-1.5ex}\end{equation}
with $z_{j,c}$ the logit of bin $c$, $[l_j,u_j]$ the range of dimension $j$, and $\Delta_j$ the width of one bin. We write $\hat{\bm{a}}=[\hat a_1,\dots,\hat a_d]^{\top}$ for the decoded action vector. The remaining dimensions are natively discrete and carry no bin width, {\eg} RT-1's three-way arm/base/terminate mode, so we set $\Delta_j{=}0$ for $j\notin\mathcal{C}$ and write $\bm{\Delta}=[\Delta_1,\dots,\Delta_d]^{\top}\in\mathbb{R}^{d}$ for the resulting vector of bin widths. 
Uniform binning fixes $\Delta_j$ independently of the local sensitivity of the task, so a single-bin logit flip always carries a bounded but non-zero physical displacement. 
\emph{Kinematic map:} Let $\bm{\theta}$ denote the current joint configuration and $J(\bm{\theta})$ the manipulator Jacobian. Let $\mathcal{J}$ index the continuous action dimensions driving the arm, excluding the gripper aperture, the base dimensions, and the discrete arm/base/terminate mode, so $|\mathcal{J}|{=}6$ for end-effector action spaces and $|\mathcal{J}|{=}n$ for an $n$-joint arm in joint space. For an action increment $\bm{u}$ supported on $\mathcal{J}$, the induced end-effector displacement over one control period $\tau$ is $G(\bm{\theta})\bm{u}\in\mathbb{R}^{6}$ with $G(\bm{\theta})\in\mathbb{R}^{6\times|\mathcal{J}|}$, where $G(\bm{\theta})=I_{6}$ for end-effector position action spaces ({\eg} RT-1, RT-2, OpenVLA, and most of \autoref{tab:multi_dataset}), $G(\bm{\theta})=\tau I_{6}$ for end-effector velocity action spaces ({\eg} ConqHose, FMB), and $G(\bm{\theta})=J(\bm{\theta})$ for joint-space action spaces ({\eg} ACT, RoboSet, SPOC), the last being the first-order approximation valid for the small increments considered here. This covers the three continuous action-space families in \autoref{tab:multi_dataset}. %VIMA's primitive-skill space carries no uniform binning and falls outside ABBP.
Because a twist mixes length and angle units, we measure displacement as $\|W G(\bm{\theta})\bm{u}\|_{2}$ with $W=\mathrm{diag}(1,1,1,\ell,\ell,\ell)$ and $\ell$ a characteristic lever arm of the embodiment.
...
\label{eq:abbp-shift}
\vspace{-1.5ex}\end{equation}
subject to $\hat{\bm{a}}+\bm{s}\odot\bm{\Delta}\in\mathcal{A}(\bm{\theta})$, where $\odot$ is the elementwise product, $\dot a_{\max}$ the per-dimension action-rate limit, and $\mathcal{A}(\bm{\theta})$ contains the actions $\bm{a}$ whose executed configuration $\psi(\bm{\theta},\bm{a})$ after one control period $\tau$ respects the joint limits $\Theta$ and the reachable workspace $\mathcal{W}$, and which satisfy the per-step velocity bound $\|\bm{a}-\hat{\bm{a}}\|_{\infty}\le\dot a_{\max}\tau$. The objective is convex in $\bm{s}$, so the $\ell_1$ budget $\beta$ alone would concentrate the whole shift in the single dimension of largest physical gain. The per-dimension cap $\sigma<\beta$ forces the shift across several degrees of freedom, and the bin bounds keep $b_j+s_j^{*}$ decodable without post-hoc clipping.
\cib{2} \emph{perturbation synthesis.} With $b_j^{*}=b_j+s_j^{*}$, ABBP solves a margin objective over the visual input,
\begin{equation}
  \small
...
\label{eq:abbp-obj}
\vspace{-1.5ex}\end{equation}
where $\bm{\delta}$ is the additive image perturbation, $\epsilon$ its $\ell_{\infty}$ budget, and $N$ the number of pixel values in $\bm{x}$, with $[\cdot]_{+}=\max(\cdot,0)$, confidence margin $\kappa$, and box constraint in raw pixel space before encoder normalization. The weights are $w_j=\|(WG(\bm{\theta}))_{:,j}\|_{2}\Delta_j$ for $j\in\mathcal{J}$, so the optimizer allocates perturbation budget to the dimensions with the largest physical effect per bin, and $w_j=\lambda$ for $j\notin\mathcal{J}$. Stage 1 fixes $s_j^{*}{=}0$ off $\mathcal{J}$, so $b_j^{*}{=}b_j$ there and the second group of terms pins the gripper aperture and the arm/base/terminate mode to their clean values. Without them $\bm{\delta}$ could flip the mode token to \emph{terminate}, which halts the episode and drives the displacement the attack optimizes for to zero. For $j\notin\mathcal{C}$ the inner maximum runs over that dimension's own label set rather than the $B$ bins. \autoref{eq:abbp-obj} treats the $d$ logit vectors as conditionally independent, which holds for parallel readout heads ({\eg} RT-1, Octo). For autoregressive action decoders ({\eg} RT-2, OpenVLA), the action tokens form a sequence and $\bm{z}_j$ depends on the preceding tokens, so we evaluate each margin under teacher forcing on the target prefix $b^{*}_{<j}$. We solve \autoref{eq:abbp-obj} by projected gradient descent with step $\alpha$ over $T$ iterations. Setting $w_j{=}1$ for $j\in\mathcal{J}$ and $\sigma{=}1$ while retaining $\mathcal{A}(\bm{\theta})$ yields a discretization-agnostic variant, isolating the contribution of the physical-effect weighting in an ablation.

\BfPara{Evaluation Protocol} Considering PGD, FGSM, and C\&W as baselines, we evaluate and benchmark ABBP using physical consequence under same budget $\epsilon$.
% A one-bin action shift and a language-token substitution are not comparable units, so we compare attacks at matched physical consequence rather than at matched $\epsilon$. 
A single bin crossing displaces the end-effector by a fraction of a millimetre, which no task-level metric resolves, so we measure consequence as the cumulative deviation of a run rather than a one-step displacement. 
From a common initial state and instruction, we run the policy for $H$ steps twice, once on clean observations and once with the attack re-optimized at every step, and, for a target displacement $\rho$, compare the two end-effector poses $(\bm{p}_k,R_k)$ and $(\bm{p}'_k,R'_k)$ at each step $k=0,\dots,H$ obtained by forward kinematics,
\begin{equation}
...
\label{eq:abbp-gate}
\vspace{-1.5ex}\end{equation}
where $\mathcal{M}$ ranges over the attacks under comparison, ${rv}$ is the rotation-vector map of the relative rotation, $E$ is a fixed geometric grid of budgets capped at $\epsilon_{\max}$, $\epsilon^{*}_{\mathcal{M}}(\rho)=\epsilon_{\max}$ when no grid point reaches $\rho$, and the expectation runs over evaluation episodes. 
%The grid avoids assuming that deviation grows monotonically in $\epsilon$, which no baseline guarantees. Cumulative measurement separates ABBP from TTDA by construction. ABBP re-solves \autoref{eq:abbp-shift} greedily at each step under a fixed per-step cap and lets deviation accumulate through the closed loop, whereas TTDA optimizes the accumulation jointly across a predicted action chunk in order to evade a sequence-level check. Whether closed-loop compounding is superlinear, and therefore whether a per-step cap of a few bins reaches a task-relevant $\rho$, is the empirical question the gate settles.
We fix $H{=}20$ steps, roughly one quarter of the median Xarm7 episode, and calibrate $(\sigma,\rho)$ against the open-loop reachability bound $\rho\le H\sqrt{|\mathcal{J}|}\,\sigma\max_{j\in\mathcal{J}}\|(WG(\bm{\theta}))_{:,j}\|_{2}\Delta_{j}$ implied by \autoref{eq:abbp-quant} and \autoref{eq:abbp-shift}, so the target remains reachable in principle before any experiment runs. We estimate $\epsilon^{*}$ independently for each of ten seeds. 
We report ABBP as a distinct vulnerability if $\epsilon^{*}_{\text{ABBP}}(\rho)\le\gamma\,\min_{\mathcal{M}\in\{\text{PGD},\text{FGSM},\text{C\&W}\}}\epsilon^{*}_{\mathcal{M}}(\rho)$ with $\gamma=0.7$ at $\rho=5$\,cm, on at least two of the three Xarm7 tasks, under a one-sided Wilcoxon signed-rank test at $p<0.05$ on the paired per-seed ratios. 
%We use a rank test because $\epsilon^{*}$ takes values on the finite grid $E$ and is not approximately normal at ten seeds. 
% If the gate fails, we record the negative result, drop the discretization hypothesis, and reallocate the T1-1 effort to TTDA and to the T1-4 defenses.

\BfPara{ABBP in Poisoning Scenarios} The two-stage design of ABBP extends from \emph{evasion} to training-time \emph{poisoning} attacks (see \autoref{table:attacks_models}). In evasion, ABBP perturbs the input under budget $\epsilon$; in poisoning, the attacker applies the target selection $\bm{s}^{*}$ (\autoref{eq:abbp-shift}) and the margin objective (\autoref{eq:abbp-obj}) to training data, action labels, or gradient updates, and we reinterpret the attack budget as the poison rate, \ie the fraction of corrupted samples or updates. For example,
because VLA action labels are bin indices, \emph{label flipping} reduces to setting $b_j \rightarrow b_j+s_j$ on the highest-gain dimensions $j\in\mathcal{J}$, where a shift of $\pm\sigma$ bins encodes a physically adjacent action and evades label inspection. 
\emph{Backdoor poisoning} pairs a visual or textual trigger with labels fixed at $b_j^{*}=b_j+s_j^{*}$, so the trigger induces a kinematically feasible displacement of maximal magnitude at inference, while \emph{clean-label} and \emph{targeted} poisoning variants keep labels intact and instead perturb the observation so the model binds a context to an off-boundary action mapping. \emph{Subpopulation poisoning} concentrates corrupted samples on configurations with high Jacobian gain $\|(WG(\bm{\theta}))_{:,j}\|_{2}$, limiting degradation on clean data outside the target subpopulation. \emph{Model poisoning}, \emph{model replacement}, and \emph{model boosting} assume gradient- or model-channel access, \eg federated fine-tuning or fine-tuning-as-a-service, and use the ABBP margin as an auxiliary training loss, biasing logits near bin boundaries while clean task success stays high. \emph{Functional attacks} follow the same pattern, poisoning the learned policy toward systematic violation of a behavioral property such as the per-step velocity bound $\|\bm{a}-\hat{\bm{a}}\|_{\infty}\le\dot a_{\max}\tau$. 
...
\subsubsection{T1-2: Gray-box Attacks} \label{sec:t1_2}
Gray-box attacks exploit partial knowledge of the victim model, such as the architecture or aspects of the training pipeline and data, and expose vulnerabilities under moderate adversarial knowledge. 
\autoref{table:attacks_models} shows possible attacks in this category, \eg \emph{prompt injection}, \emph{refusal suppression}, \emph{style injection}, \emph{role-play strategies}, and \emph{jailbreak} for evasion, and \emph{membership}, \emph{property}, and \emph{attribute inference} for privacy.

\BfPara{Related Work} Existing gray-box attacks use publicly available vision or language encoders as surrogates. The attacker aligns the perturbation with surrogate embeddings and transfers the result to the victim VLM, either to produce adversarial semantics or to conceal the perturbation for imperceptibility.
Zhao \etal~\cite{zhao2024evaluating} use pre-trained CLIP~\cite{radford2019language} and BLIP~\cite{li2022blip} as surrogates to align adversarial perturbations with textual and image embeddings and generate transferable adversarial examples.
Wang \etal~\cite{wang2024transferable} show attention-directed feature perturbation can guide highly transferable attacks by disrupting modality-consistency features in critical attention regions.
Wu \etal~\cite{wu2024adversarial} use CLIP~\cite{radford2019language} as a surrogate to attack GPT-4o, Gemini, and Claude, embedding adversarial sub-images within larger images.
None of these attacks targets VLAs or measures action-space consequences. %Prior work reports transfer success on the text or image outputs of VLMs, not on the actions of a robot policy, and relies on fixed public surrogates with no control over the surrogate training recipe.
\ul{In this sub-task, we propose  new attack family, Surrogate-Transfer Action Perturbation (STAP), and we study gray-box privacy attacks against robotic models.}

\BfPara{Proposed Attack} We propose STAP, a surrogate-transfer gray-box attack grounded in the action space of the robot.
We define the gray-box threat model as access to the public pre-trained components of the victim policy (\ie the vision encoder, the VLM backbone, and the action tokenizer; see \autoref{tab:VLMs}) and knowledge of the architecture family, without access to the fine-tuned policy weights, the action head, or the data mixture. This assumption matches deployed systems, where vendors release base models (\eg Octo and OpenVLA) while embodiment-specific fine-tuned checkpoints and training data remain private. 
STAP extends ABBP to this setting. The attacker solves target selection (\autoref{eq:abbp-shift}) and the margin objective (\autoref{eq:abbp-obj}) on a surrogate policy $\mathcal{F}'$, then transfers the perturbation to the victim $\mathcal{F}$. Because we control the surrogate's backbone, training data, and embodiment head, we vary each factor independently and measure how surrogate-victim mismatch in backbone, data mixture, and embodiment degrades transfer, a study not possible with fixed public surrogates. 
Language-modality attacks (\eg \emph{prompt injection} and \emph{jailbreak} through surrogate text embeddings) apply only to language-conditioned models (\ie RT-1, RT-2, OpenVLA, and Octo) and exclude vision-only models such as ACT. 

\BfPara{STAP in Privacy Attacks} For \emph{membership inference}, we probe the victim with episodes drawn from the training mixture (members) and with held-out demonstrations (non-members), and we score the gap in action-token confidence between the two sets. For \emph{attribute} and \emph{property inference}, we train a probe on surrogate action traces to recover attributes of the victim's training data, \eg embodiment and task composition, from the predicted actions. Both attacks use the surrogate to calibrate the decision threshold before querying the victim, keeping the query budget small.

\BfPara{Evaluation} We evaluate STAP across various surrogate-victim pairs in the transferability matrix under various physical-consequence budget $\epsilon^{*}(\rho)$ (\autoref{eq:abbp-gate}). We report transfer success per model pair, query budget where applicable, and membership-inference advantage (true-positive rate at 1\% false-positive rate) for the privacy attacks, each as mean~$\pm$~std over at least ten runs.
...
% Tan \etal \cite{tan2024wolf} explore the indirect propagation of malicious content by subtly influencing a single 'wolf' agent to compel other 'sheep' agents in the society to produce malicious content.
...
% Wang \etal \cite{wang2024stop} evaluate the adversarial robustness of MLLMs employing chain of thought (CoT) reasoning, finding that models using CoT generally exhibit significantly higher robustness under both answer and rationale attacks compared to models without CoT.
...
% Guo \etal \cite{guo2024efficiently} utilize diffusion models to generate natural, unrestricted adversarial examples.
...
% Dong \etal \cite{dong2023robust} investigate the adversarial robustness of Google's Bard as a representative of VLMs through transfer-based attacks, demonstrating that these adversarial images are highly transferable and effective at deceiving other VLMs.
...
\subsubsection{Evaluation Plan and Deliverables} \label{sec:t1_evaluation}
