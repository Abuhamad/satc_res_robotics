...
\BfPara{Robotics Data Mixture} We consider multimodal robotics data mixture 
for six robots, six embodiments (\textit{fewer than the 22 included in the OXE~\cite{9} dataset for multimodality and practical reasons}), and 15 $(=$14$+$OXE) datasets across all experiments, as shown in~\autoref{tab:multi_dataset}.
...
Note, we plan to continue training policies on the extended versions (\ie added tasks and episodes) of our dataset as we continue to grow the dataset. 
...
{In the following, we describe the main models that we will use in this project. Note that we will continue to expand the list of models as new models are released and/or become available for research use.}
...
% VLMs, 
% including Flamingo \cite{alayrac2022flamingo}, OpenFlamingo \cite{awadalla2023openflamingo}, SPHINX-X \cite{gao2024sphinx},
% Qwen-VL \cite{bai2023qwen},
% BLIP-2 \cite{li2023blip}, 
% InstructBLIP \cite{dai2024instructblip}, BLIVA \cite{hu2024bliva},
% MiniGPT-4 \cite{zhu2023minigpt}, 
% MiniGPT-v2 \cite{chen2023minigpt}, 
% LLaVA \cite{liu2024visual}, 
% LLaVA-1.5 \cite{liu2024improved},  
% LLaMA Adapter-V2 \cite{gao2023llama}, and PandaGPT \cite{su2023pandagpt}. 
...
conditional variational autoencoder (CVAE)~\cite{sohn2015learning} that generates a sequence of actions.
The original ACT was designed to process visual data and joint positions without additional language instructions. 
...
% \noindent\textbf{{\cib{$\star$}} Different Model: Horizon Gaze.} 
...
The attack operation $\mathcal{P} (.)$ is defined as a perturbation function that alters the inputs (one or more modalities, such as images, text, and sensory data) to mislead the model into producing an incorrect/unintended output action.
...
\ul{In this task, we explore many threat models that span different categorizations and targets as shown in}{~\autoref{table:attacks_models}}.



\BfPara{Attacking Robotic Foundation Models: Preliminary Study} We have co-\\
{\vspace{-2.5ex}}
...
The results are shown in~\autoref{fig:visual_black_box}, where we observe that the success rate of the models drops significantly after the attack, indicating that even simple token-based perturbations can disrupt the model's performance.
...
% {\vspace{-1ex}}
\begin{wraptable}{r}{0.36\textwidth}{\vspace{-2ex}}
\centering
...
\BfPara{Staged Scope} We stage the program into a Minimum Viable Research (MVP) phase in Years~1 and~2 and a scale-up phase in Years~3 and~4. The MVP phase targets two open, architecturally complementary models (OpenVLA and Octo) on two embodiments (the Xarm7 real arm and the Google robot in the Simpler~\cite{li24simpler} simulator), and develops ABBP and TTDA with ported baselines together with the TACD and AGAT defenses and their baselines. The scale-up phase adds a flow-matching family (pi-0~\cite{black2024pi_0} or OpenVLA-OFT), a morphologically distinct embodiment (Franka or Spot), the T2 multimodal track, certified-robustness defenses, and the full public release of VLA-SecBench. The MVP still supports generalizable claims because OpenVLA and Octo span the autoregressive and diffusion policy families, and the Xarm7 and Simpler pairing spans real and simulated embodiments.

\BfPara{Timeline} \autoref{fig:workplan_timeline} summarizes the work-package schedule under the calendar aligned to the budget period.
...
\begin{tikzpicture}[font=\scriptsize\sffamily,
  ...
  \fill[gray!7]  (0,-3.5) rectangle (8,1.4);
  \fill[gray!14] (8,-3.5) rectangle (16,1.4);

  %%% horizontal row separators %%%
  \foreach \hy in {0.65,-0.05,-0.75,-1.45,-2.15,-2.85}
    \draw[very thin, gray!25] (0,\hy) -- (16,\hy);

  %%% vertical quarter gridlines %%%
  \foreach \gx in {1,2,3,5,6,7,9,10,11,13,14,15}
    \draw[very thin, gray!25] (\gx,-3.5) -- (\gx,1.4);
  %%% year boundary lines %%%
  \foreach \gx in {4,8,12}
    \draw[thin, black!40] (\gx,-3.5) -- (\gx,1.4);
  %%% outer chart border %%%
  \draw[black!60, rounded corners=1pt] (0,-3.5) rectangle (16,1.4);
  ...
  %%% left-hand labels %%%
  \node[anchor=east, font=\scriptsize\bfseries\sffamily] at (-0.3,2.2)  {Work package};
  \node[anchor=east] at (-0.3,1.0)  {WP0 Infrastructure \& repository};
  \node[anchor=east] at (-0.3,0.3)  {WP1 Attack primitives (Aim 1)};
  \node[anchor=east] at (-0.3,-0.4) {WP2 Defense validation (Aim 2)};
  \node[anchor=east] at (-0.3,-1.1) {WP3 Evaluation rigor};
  \node[anchor=east] at (-0.3,-1.8) {WP4 Multimodal attacks (Aim 3)};
  \node[anchor=east] at (-0.3,-2.5) {WP5 Benchmark \& transferability (Aim 4)};
  \node[anchor=east] at (-0.3,-3.2) {WP6 Education \& broader impacts};

  %%% work-package bars (active quarters) %%%
  \fill[bar] (0,0.76)  rectangle (3,1.24);   % WP0: Y1Q1--Q3
  \fill[bar] (1,0.06)  rectangle (6,0.54);   % WP1: Y1Q2--Y2Q2
  \fill[bar] (2,-0.64) rectangle (8,-0.16);  % WP2: Y1Q3--Y2Q4
  \fill[bar] (1,-1.34) rectangle (5,-0.86);  % WP3: Y1Q2--Y2Q1
  \fill[bar] (8,-2.04) rectangle (12,-1.56); % WP4: Y3Q1--Q4
  \fill[bar] (9,-2.74) rectangle (16,-2.26); % WP5: Y3Q2--Y4Q4
  \fill[bar] (5,-3.44) rectangle (16,-2.96); % WP6: Y2Q2--Y4Q4
  ...
  \foreach \mx/\ml in {0.5/M0, 1.5/M1, 2.5/M2, 4.5/M3, 5.5/M4a, 6.5/M4, 7.5/M5, 9.5/M6, 13.5/M7} {
    \draw[very thin, gray!45] (\mx,-3.5) -- (\mx,-3.63);
    \fill[black, draw=black] (\mx,-3.73) -- (\mx-0.11,-3.85) -- (\mx,-3.97) -- (\mx+0.11,-3.85) -- cycle;
    \node[font=\tiny\sffamily, anchor=north, inner sep=1pt] at (\mx,-4.0) {\ml};
  }

  %%% phase captions %%%
  \node[font=\tiny\itshape\sffamily, text=black!60] at (4,-4.4)  {MVP phase (Years 1--2)};
  \node[font=\tiny\itshape\sffamily, text=black!60] at (12,-4.4) {Scale-up phase (Years 3--4)};
\end{tikzpicture}%
}
\caption{Gantt-style work-plan timeline. Calendar aligned to the budget period: Year~1 = Sep~2026--Aug~2027, \ldots, Year~4 = Sep~2029--Aug~2030; quarters Q1~(Sep--Nov), Q2~(Dec--Feb), Q3~(Mar--May), Q4~(Jun--Aug). Bars indicate active quarters; diamonds mark milestones (M0--M7).}
\label{fig:workplan_timeline}
\end{figure*}

\BfPara{Milestones and Go/No-Go Gates}
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}\itemindent=-1em
\item \textbf{M0 (Y1Q1):} NSF ACCESS Explore allocation or an equivalent institutional GPU node secured. NO-GO: restrict white-box evaluation to the encoder-only threat model and resubmit the allocation request.
\item \textbf{Co-PI confirmation (pre-submission):} Co-PI confirmed with a signed collaboration letter and documented Xarm7 access before submission.
\item \textbf{M1 (Y1Q2):} ABBP formal specification complete. NO-GO: reframe Aim~1 around TTDA as the primary novel primitive.
\item \textbf{M2 (Y1Q3):} MVP model zoo operational on at least ten tasks, reproducing preliminary baselines.
\item \textbf{M3 (Y2Q1):} ABBP versus PGD comparison shows a statistically significant advantage at matched, physically-normalized budget. NO-GO: reframe ABBP as an architectural-analysis finding.
\item \textbf{M4a (Y2Q2):} preliminary TACD false-positive check on benign demonstrations. NO-GO: revise the threshold strategy before full defense-evaluation runs.
\item \textbf{M4 (Y2Q3):} formal TACD detection/false-positive gate.
\item \textbf{M5 (Y2Q4):} VLA-SecBench v0.5 prototype, and pi-0 (or an equivalent open flow-matching VLA) availability confirmed. NO-GO on availability: substitute an Octo-large diffusion variant and document the substitution.
\item \textbf{M6 (Y3Q2):} multimodal coupling benefit over unimodal baselines demonstrated.
\item \textbf{M7 (Y4Q2):} VLA-SecBench v1.0 ready for release with dual-use review complete. Interpretability-tool validation is completed earlier, at Y3Q4, to reduce Year-4 load.
\end{itemize}

\BfPara{Division of Labor} \autoref{tab:division_labor} assigns parallel security and robotics tracks so the PI and Co-PI progress without blocking each other.

\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{Division of labor across the two-investigator team.}
\label{tab:division_labor}
\begin{tabular}{p{0.46\linewidth} p{0.46\linewidth}}
\toprule
\textbf{PI (AI/ML security)} & \textbf{Co-PI [Co-PI: TBD] (robot learning)} \\
\midrule
Attack formal design and analysis (ABBP, TTDA theory) & Real-robot infrastructure and Xarm7 setup \\
Defense design and analysis (AGAT, ported defenses) & TTDA/TACD real-robot implementation and false-positive testing \\
Benchmark design, transferability matrix, statistics & Physical-metric instrumentation (end-effector displacement, force/safety zones) \\
Multimodal coupling design (T2), threat taxonomy & Real-robot experiments, physical patches, scale-up embodiment integration \\
Curriculum lead, publication lead & Adversarial-robotics lab (robot component) \\
\bottomrule
\end{tabular}
\end{table}

\BfPara{Compute Plan} ABBP gradient evaluation targets OpenVLA's released visual encoders (DINOv2 and SigLIP, roughly 300M parameters combined), AGAT uses LoRA rank-16 adaptation, and the team will apply for an NSF ACCESS Explore allocation in Y1Q1 as the first infrastructure milestone.
