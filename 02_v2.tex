...
\subsection{T0: Ongoing Progress: Comprehensive Repository for Robotic Models and Data}
% ~\vspace{-3ex}
...
\emph{Comparison protocol.} PGD, FGSM, and C\&W serve as baselines, applied per step under the same budget. A one-bin action shift and a language-token substitution are not comparable units, so we compare attacks at matched physical consequence rather than at matched $\epsilon$. A single bin crossing displaces the end effector by a fraction of a millimetre, which no task-level metric resolves, so we measure consequence as the cumulative deviation of a closed-loop rollout rather than a one-step displacement. From a common initial state and instruction, we roll out the policy for $H$ steps twice, once on clean observations and once with the attack re-optimized at every step, and, for a target displacement $\rho$, compare the two end-effector poses $(\bm{p}_k,R_k)$ and $(\bm{p}'_k,R'_k)$ at each step $k=0,\dots,H$ obtained by forward kinematics,
\begin{equation}
\bm{e}_{k}=\begin{bmatrix}\bm{p}'_{k}-\bm{p}_{k}\\[2pt]\mathrm{Log}\bigl(R_{k}^{\top}R'_{k}\bigr)\end{bmatrix},
\qquad
...
\label{eq:abbp-gate}
\end{equation}
where $\mathcal{M}$ ranges over the attacks under comparison, $\mathrm{Log}$ is the rotation-vector map of the relative rotation, $E$ is a fixed geometric grid of budgets capped at $\epsilon_{\max}$, $\epsilon^{*}_{\mathcal{M}}(\rho)=\epsilon_{\max}$ when no grid point reaches $\rho$, and the expectation runs over evaluation episodes. The grid avoids assuming that deviation grows monotonically in $\epsilon$, which no baseline guarantees. Cumulative measurement separates ABBP from TTDA by construction. ABBP re-solves \autoref{eq:abbp-shift} greedily at each step under a fixed per-step cap and lets deviation accumulate through the closed loop, whereas TTDA optimizes the accumulation jointly across a predicted action chunk in order to evade a sequence-level check. Whether closed-loop compounding is superlinear, and therefore whether a per-step cap of a few bins reaches a task-relevant $\rho$, is the empirical question the gate settles.

\emph{Go/no-go gate (M1, M3).} We frame ABBP as research to be conducted. We fix $H{=}20$ steps, roughly one quarter of the median Xarm7 episode, and calibrate $(\sigma,\rho)$ against the open-loop reachability bound $\rho\le H\sqrt{|\mathcal{J}|}\,\sigma\max_{j\in\mathcal{J}}\|(WG(\bm{\theta}))_{:,j}\|_{2}\Delta_{j}$ implied by \autoref{eq:abbp-quant} and \autoref{eq:abbp-shift}, so the target remains reachable in principle before any experiment runs. We estimate $\epsilon^{*}$ independently for each of ten seeds. At M3 we declare a structurally distinct vulnerability only if $\epsilon^{*}_{\text{ABBP}}(\rho)\le\gamma\,\min_{\mathcal{M}\in\{\text{PGD},\text{FGSM},\text{C\&W}\}}\epsilon^{*}_{\mathcal{M}}(\rho)$ with $\gamma=0.7$ at $\rho=5$\,cm, on at least two of the three Xarm7 tasks, under a one-sided Wilcoxon signed-rank test at $p<0.05$ on the paired per-seed ratios. We use a rank test because $\epsilon^{*}$ takes values on the finite grid $E$ and is not approximately normal at ten seeds. If the gate fails, we record the negative result, drop the discretization hypothesis, and reallocate the T1-1 effort to TTDA and to the T1-4 defenses.
