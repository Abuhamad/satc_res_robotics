...
\begin{minipage}{0.26\textwidth}
  \centering
  \includegraphics[width=0.74\textwidth]{figures/results/lang_white_box.pdf}\vspace{-2ex}
...
\label{eq:abbp-gate}
\vspace{-1.5ex}\end{equation}
where where $\mathcal{M}$ ranges over the attacks under comparison, ${rv}$ is the rotation-vector map of the relative rotation, 
...
The $\epsilon^{*}_{\mathcal{M}}(\rho)=\min\{\epsilon\in E:\;\widehat{\mathbb{E}}[D_{\mathcal{M}}(H)]\ge\rho\}$ defines the smallest budget $\epsilon$ on grid $E$ at which attack $\mathcal{M}$ achieves a consequence meeting or exceeding $\rho$.
We consider a proposed attack $\mathcal{M}_b$ as a distinct vulnerability over attacks (\eg $\mathcal{M}_1,\dots,\mathcal{M}_m$) if $\epsilon^{*}_{{\mathcal{M}_b}}(\rho)\le\gamma\,\min_{\mathcal{M}\in\{\mathcal{M}_1,\dots,\mathcal{M}_m\}}\epsilon^{*}_{\mathcal{M}}(\rho)$.
As adversarial knowledge weakens, $\gamma$ relaxes: $\gamma{=}0.7$ (white-box, \autoref{sec:t1_1}), $\gamma_{g}{=}0.8$ (gray-box, \autoref{sec:t1_2}), $\gamma_{b}{=}0.9$ (black-box, \autoref{sec:t1_3}). Training-time attacks use the poison rate as budget. Metrics are reported as mean~$\pm$~std across runs. 
...
\BfPara{Proposed Attack} While we continue to invistegate language-based attacks (\eg SHIELD \autoref{fig:visual_black_box}), we propose vision-based attack, TTDA.
